...
import static j2me.wrapper.linux.MIDletManager.WORKDIR;
...
public class MIDletList extends JPanel {
    ...
    public MIDletList() {
        ...
        File[] fileList = (new File(WORKDIR + "apps")).listFiles(new FilenameFilter() {
        ...
        for (int i = 0; i < fileList.length; i++) {
            ...
            appBtn.setPreferredSize(new Dimension(200, MIDletManager.BTN_H));
            // bigger font
            appBtn.setFont(new Font(appBtn.getFont().getFontName(), Font.PLAIN,
                    appBtn.getFont().getSize() * 3));
            
            // try to load and set app icon
            String iconPath = WORKDIR + "apps/" + midletName + ".png";
            try {
